...
// change credit balance reached a critical level
$where = array();
$where['type'] = 'product';
$where['name'] = $_LANG['onappuserslowbalanceemail'];
if (!mysql_num_rows(select_query('tblemailtemplates', 'id', $where))) {
    $fields = array();
    $fields['type'] = 'product';
    $fields['name'] = $fields['subject'] = $where['name'];
    $fields['message'] = '<p>Dear {$client_name},</p>
                    <p>Your credit balance reached a critical level {$client_credit}.<br />
                    Please add some credits or your account will be suspended.</p>';
    $fields['plaintext'] = 0;
    insert_query('tblemailtemplates', $fields);
}
